...
COMPARATIVAS COMO IMAGEN: las comparativas van SIEMPRE como imagen diseñada generada con node-canvas (npm canvas), NO como tabla HTML. Flujo: crear HTML estilizado → screenshot con node-canvas → subir PNG a Drive (API directa, hacer público) → embeber en el doc con <img src="URL">. Diseño: fondo card con gradiente, header azul #0B149D, filas alternas, columna "Con CarbonBox" en verde #E8F1EC, footer verde #00C269 con CTA.
...
5. ROTACIÓN DE RESPONSABLE: lee "1_Estrategia/rotacion_responsables.json". responsable = orden[proximo_index]. Tras crear el evento, actualiza el archivo: proximo_index = (proximo_index + 1) % longitud(orden), ultimo_asignado = responsable, ultima_actualizacion = hoy; y guárdalo.
